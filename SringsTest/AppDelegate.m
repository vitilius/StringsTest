...
//  SringsTest
//
//  Created by Viktor Lukashevich on 18.04.16.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSMutableArray *invertedWords = [[NSMutableArray alloc] init];
    NSLog(@"%@",wordsArray);
   
    NSString *charList = @".,;/”()";
    for (NSString *word in wordsArray) {
        
        NSInteger lastIndex =[word length] - 1;
        NSInteger firstIndex = 0;
        NSString* lastChar = [word substringFromIndex:lastIndex];
        NSString* firstChar = [word substringFromIndex:0];

        NSRange range1 = [charList rangeOfString:lastChar];
        NSRange range2 = [charList rangeOfString:lastChar];
        ...
        NSRange lastRange = NSMakeRange(lastIndex, 1);
        NSRange firstRange = NSMakeRange(firstIndex, 1);
        
        lastChar = [[word substringWithRange:lastRange] uppercaseString];
        firstChar = [[word substringWithRange:firstRange] lowercaseString];
        
        NSMutableString *mutableWord  = [NSMutableString stringWithString:word];
        [mutableWord replaceCharactersInRange:lastRange withString:lastChar];
        [mutableWord replaceCharactersInRange:firstRange withString:firstChar];

        [invertedWords addObject:mutableWord];
        
        text = [invertedWords componentsJoinedByString:@" "];
    }
